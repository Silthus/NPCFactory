...
import de.raidcraft.npc.api.EquipmentSlot;
import de.raidcraft.npc.api.NPC;
import de.raidcraft.npc.api.NPCAnimation;
import de.raidcraft.npc.api.NPCProfile;
import de.raidcraft.npc.api.events.NPCDamageEvent;
import de.raidcraft.npc.api.events.NPCInteractEvent;
...
public class NPCEntity extends EntityPlayer implements NPC {
    ...
    private void broadcastLocalPacket(Packet packet) {
        ...
        getBukkitEntity().getWorld().getPlayers().stream()
                .filter(player -> getBukkitEntity().getLocation().distanceSquared(player.getLocation()) <= RADIUS * RADIUS)
                .forEach(craftPlayer -> ((CraftPlayer) craftPlayer).getHandle().playerConnection.sendPacket(packet));
    }
